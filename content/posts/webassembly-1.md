...
Web assembly "WASM" is an assembly language that targets the javascript virtual machine.  It can be run in both modern web browsers as well as other VM enviornments such as `node` or (Spidermonkey???). The primary advantage of WASM over traditional javascript is performance.  How can WASM be faster than Javascript if it runs on the same Virtual Machine? There are many reasons to why WebAssembly _could_ be faster and I am not going to go into them all here, but one of the priamry reason is that you normally use a compiler to convert some higher level language such as C/C++ to WebAseembly.  The compiler can apply all of the standard optimizations that you would expect from a good compiler prior to making use of the VM JIT optimizations.
...
You can check if your clang/llvm supports wasm by checking if `wasm32` is a registered target backend in the `LLVM static compiler`.
...
# Add the bin directoy to our path
...
Uhh... This isn't your typical helloworld.  Where is `out` defined?  Why aren't we using `printf` or `puts`?  Well for our simple example here we will be targeting an enviornment that doesn't actually have a `libc` available for us.  This enviornment is also missing `POSIX` and your standard OS syscalls.  For now we will simply define an extern function named `out`.  Once we go through the compliation steps and start interfacing with the browser we will go over how/where this function is defined. 


## Bulding helloworld.c
...
Great!  We now have a WebAssembly binary.  But how do I use this thing?  Well first lets examine it.
...
Ok... that wasn't that useful.  Lets convert the binary to some human readable form.  One of the more popular representations of this is known as `WAT` or the `WebAssembly text format`.  IMHO its an interesting way to actually represent the binary since it uses S-expressions.  For this we will need to use the `wasm2wat` tool that comes with `The WebAssembly Binary Toolkit`.  
...
Converting our binary to the `WAT` format. I have added some commentes here, but in order to better understand this format I suggest you referrer to the offical documentation here: <https://developer.mozilla.org/en-US/docs/WebAssembly/Understanding_the_text_format>
...
# FIXME!!! is .L.str storing data in global section?

## Loading WASM in the browser


So we now have a WASM binary, but how do we use it? The below snipit shows how to put everything together.  
...
With our `out.wasm` and `helloworld.html` in the same directory we can start a webserver and visit in our demo in a browser that supports `WebAssembly`.   You should see "Hello World" written on your screen! Browser support can be found here: <https://developer.mozilla.org/en-US/docs/WebAssembly>


Obviously this is a lot of work just to print "Hello World" to the screen and is clearly not the best use case for WASM, but it does serve as a good starting point in understanding how all the peices fit together. 


# helloworld.c to WASM the easy way

The example above required a lot of steps to get a simple example working.  Not having a `libc` available to use makes porting any code challenging.  Luckily there is already a project that fills this gap: [Emscripten](http://kripken.github.io/emscripten-site).  Emscripten is an entire toolchain/enviornment that comes with everything you need to get started, including a port of `libc` using [musl](https://www.musl-libc.org/).  It also comes with a few other ports that make porting/building an app targeting WASM easier.  
...
Emscripten is well documented so I am not going to cover that here.
...
<https://stackoverflow.com/questions/41653792/disable-linking-libc-in-emscripten>
